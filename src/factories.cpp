...
namespace pam_interface
{
  ...
  namespace four_dofs
  {
	...
	robot.joints[dof].encoder_multiplier = ENCODER_MULTIPLIERS[dof];
      ...
      for(int dof=0;dof<robot.nb_dofs;dof++){
	robot.joints[dof].stand_pressure[0] = STAND_PRESSURES[stand_pressure_index];
	stand_pressure_index++;
	robot.joints[dof].stand_pressure[1] = STAND_PRESSURES[stand_pressure_index];
	stand_pressure_index++;
      }
	...
	for(int dof=0;dof<robot.nb_dofs;dof++){
	  ...
	  robot.joints[dof].current_pressures[sign] = CURRENT_PRESSURES[memory_index];
