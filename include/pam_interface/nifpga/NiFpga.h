 ...
 */
    ...
    defined(__x86_64) || defined(__IA32__) || defined(_X86_) ||         \
    defined(__THW_INTEL__) || defined(__I86__) || defined(__INTEL__) || \
    defined(__X86__) || defined(__386__) || defined(__I86__) ||         \
    defined(M_I386) || defined(M_I86) || defined(_M_I386) || defined(_M_I86)
#if defined(_WIN32) || defined(_WIN64) || defined(__WIN32__) ||          \
    defined(__TOS_WIN__) || defined(__WINDOWS__) || defined(_WINDOWS) || \
 ...
 */
#if NiFpga_ExactWidthIntegerTypesDefined || defined(_STDINT) ||          \
    defined(_STDINT_H) || defined(_STDINT_H_) || defined(_INTTYPES_H) || \
    defined(_INTTYPES_H_) || defined(_SYS_STDINT_H) ||                   \
    defined(_SYS_STDINT_H_) || defined(_SYS_INTTYPES_H) ||               \
    defined(_SYS_INTTYPES_H_) || defined(_STDINT_H_INCLUDED) ||          \
    defined(_MSC_STDINT_H_) || defined(_PSTDINT_H_INCLUDED)
...
extern "C"
{
    ...
    /**
     * Represents the resulting status of a function call through its return
     * value. 0 is success, negative values are errors, and positive values are
     * warnings.
     */
    ...
    /**
     * No transfer is in progress because the transfer was aborted by the
     * client. The operation could not be completed as specified.
     */
    ...
    /**
     * A parameter to a function was not valid. This could be a NULL pointer, a
     * bad value, etc.
     */
    static const NiFpga_Status NiFpga_Status_InvalidParameter = -52005;

    /**
     * A required resource was not found. The NiFpga.* library, the RIO
    ...
    /**
     * An error was detected in the communication between the host computer and
     * the FPGA target.
     */
     ...
     * the depth of the host memory DMA FIFO, or more elements were requested
     * for release than had been acquired.
     */
    static const NiFpga_Status NiFpga_Status_BadReadWriteCount = -61073;

    /**
     * A hardware clocking error occurred. A derived clock lost lock with its
     * base clock during the execution of the LabVIEW FPGA VI. If any base
     * clocks with derived clocks are referencing an external source, make sure
     * that the external source is connected and within the supported frequency,
     * jitter, accuracy, duty cycle, and voltage specifications. Also verify
     * that the characteristics of the base clock match the configuration
     * specified in the FPGA Base Clock Properties. If all base clocks with
     * derived clocks are generated from free-running, on-board sources, please
     * contact National Instruments technical support at ni.com/support.
     */
     ...
     * activities on the FPGA before requesting this operation. If the target is
     * in Scan Interface programming mode, put it in FPGA Interface programming
     * mode.
     */
    static const NiFpga_Status NiFpga_Status_FpgaBusy = -61141;

    /**
     * The operation could not be performed because the FPGA is busy operating
     * in FPGA Interface C API mode. Stop all activities on the FPGA before
     * requesting this operation.
     */
    static const NiFpga_Status NiFpga_Status_FpgaBusyFpgaInterfaceCApi = -61200;

    /**
     * The chassis is in Scan Interface programming mode. In order to run FPGA
     * VIs, you must go to the chassis properties page, select FPGA programming
     * mode, and deploy settings.
     */
    static const NiFpga_Status NiFpga_Status_FpgaBusyScanInterface = -61201;

    /**
     * The operation could not be performed because the FPGA is busy operating
     * in FPGA Interface mode. Stop all activities on the FPGA before requesting
     * this operation.
     */
    ...
    /**
     * The operation could not be performed because the FPGA is busy operating
     * in Interactive mode. Stop all activities on the FPGA before requesting
     * this operation.
     */
    ...
    /**
     * The operation could not be performed because the FPGA is busy operating
     * in Emulation mode. Stop all activities on the FPGA before requesting this
     * operation.
     */
    ...
    static const NiFpga_Status
        NiFpga_Status_ResetCalledWithImplicitEnableRemoval = -61211;
     ...
     */
    static const NiFpga_Status
        NiFpga_Status_AbortCalledWithImplicitEnableRemoval = -61212;

    /**
     * LabVIEW FPGA does not support Close and Reset if Last Reference for
     * bitfiles that allow removal of implicit enable signals in single-cycle
     * Timed Loops. Pass the NiFpga_CloseAttribute_NoResetIfLastSession
     * attribute to NiFpga_Close instead of 0.
     */
    static const NiFpga_Status
        NiFpga_Status_CloseAndResetCalledWithImplicitEnableRemoval = -61213;

    /**
     * For bitfiles that allow removal of implicit enable signals in
     * single-cycle Timed Loops, LabVIEW FPGA does not support this method prior
     * to running the bitfile.
     */
    ...
    /**
     * Bitfiles that allow removal of implicit enable signals in single-cycle
     * Timed Loops can run only once. Download the bitfile again before
     * re-running the VI.
     */
    static const NiFpga_Status
     ...
     * of unacquired elements left in the host memory DMA FIFO. There are
     * currently fewer unacquired elements left in the FIFO than are being
     * requested. Release some acquired elements before acquiring more elements.
     */
    static const NiFpga_Status
        NiFpga_Status_ElementsNotPermissibleToBeAcquired = -61219;
    ...
    /**
     * The NI-RIO driver was unable to allocate memory for a FIFO. This can
     * happen when the combined depth of all DMA FIFOs exceeds the maximum depth
     * for the controller, or when the controller runs out of system memory. You
     * may be able to reconfigure the controller with a greater maximum FIFO
     * depth. For more information, refer to the NI KnowledgeBase article
     * 65OF2ERQ.
     */
    static const NiFpga_Status NiFpga_Status_TotalDmaFifoDepthExceeded = -63003;

    /**
     * Access to the remote system was denied. Use MAX to check the Remote
     * Device Access settings under Software>>NI-RIO>>NI-RIO Settings on the
     * remote system.
     */
    static const NiFpga_Status NiFpga_Status_AccessDenied = -63033;

    /**
     * The NI-RIO software on the host is not compatible with the software on
     * the target. Upgrade the NI-RIO software on the host in order to connect
     * to this target.
     */
     ...
     * Ensure that the device is on and accessible over the network, that NI-RIO
     * software is installed, and that the RIO server is running and properly
     * configured.
     */
    static const NiFpga_Status NiFpga_Status_RpcConnectionError = -63040;

    /**
     * The RPC session is invalid. The target may have reset or been rebooted.
     ...
     */
    static const NiFpga_Status NiFpga_Status_FifoElementsCurrentlyAcquired =
        -63083;
     ...
     * this target.
     */
    static const NiFpga_Status NiFpga_Status_ControlOrIndicatorTooLarge =
        -63085;

    /**
     * A valid .lvbitx bitfile is required. If you are using a valid .lvbitx
     * bitfile, the bitfile may not be compatible with the software you are
     * using. Determine which version of LabVIEW was used to make the bitfile,
     * update your software to that version or later, and try again.
     */
    static const NiFpga_Status NiFpga_Status_BitfileReadError = -63101;

    /**
     * The specified signature does not match the signature of the bitfile. If
     * the bitfile has been recompiled, regenerate the C API and rebuild the
     * application.
     */
    ...
    /**
     * Either the supplied resource name is invalid as a RIO resource name, or
     * the device was not found. Use MAX to find the proper resource name for
     * the intended device.
     */
    ...
    /**
     * The maximum number of open FPGA sessions has been reached. Close some
     * open sessions.
     */
    ...
    /**
     * Tests whether a status is not an error. Success and warnings are not
     * errors.
     *
     * @param status status to check for an error
     * @return whether the status was a success or warning
     */
    static NiFpga_Inline NiFpga_Bool
    NiFpga_IsNotError(const NiFpga_Status status)
    {
        return status >= NiFpga_Status_Success;
    }
...
#define NiFpga_IfIsNotError(status, expression) \
    ...
    /**
     * You must call this function before all other function calls. This
     * function loads the NiFpga library so that all the other functions will
     * work. If this function succeeds, you must call NiFpga_Finalize after all
     ...
     *
     * Because different operating systems have different default current
     * working directories for applications, you must pass an absolute path for
     * the bitfile parameter. If you pass only the filename instead of an
     * absolute path, the operating system may not be able to locate the
     * bitfile. For example, the default current working directories are
     * C:\ni-rt\system\ for Phar Lap ETS and /c/ for VxWorks. Because the
     * generated *_Bitfile constant is a #define to a string literal, you can
     * use C/C++ string-literal concatenation to form an absolute path. For
     * example, if the bitfile is in the root directory of a Phar Lap ETS
     * system, pass the following for the bitfile parameter.
     *
     *    "C:\\" NiFpga_MyApplication_Bitfile
     *
     * @param bitfile path to the bitfile
     * @param signature signature of the bitfile
     * @param resource RIO resource string to open ("RIO0" or
     * "rio://mysystem/RIO")
    ...
    /**
     * Closes the session to the FPGA. The FPGA resets unless either another
     * session is still open or you use the
     * NiFpga_CloseAttribute_NoResetIfLastSession attribute.
     *
     ...
     * NiFpga_RunAttribute_WaitUntilDone, NiFpga_Run blocks the thread until the
     * FPGA finishes running.
     *
     * @param session handle to a currently open session
    ...
    /**
     * Reads an entire array of unsigned 16-bit integer values from a given
    ...
    /**
     * Reads an entire array of unsigned 32-bit integer values from a given
     * array indicator or control.
     *
     * @warning The size passed must be the exact number of elements in the
     *          indicator or control.
    ...
    /**
     * Reads an entire array of unsigned 64-bit integer values from a given
     * array indicator or control.
     *
     * @warning The size passed must be the exact number of elements in the
     *          indicator or control.
     ...
     * @param session handle to a currently open session
     * @param fifo FIFO to configure
     * @param requestedDepth requested number of elements in the host memory
     * part of the DMA FIFO
     * @param actualDepth if non-NULL, outputs the actual number of elements in
     * the host memory part of the DMA FIFO, which may be more than the
     * requested number
     * @return result of the call
     */
     ...
     *                          remaining in the host memory part of the DMA
     * FIFO
     * @return result of the call
     */
     ...
     *                               elements remaining in the host memory part
     * of the DMA FIFO
     * @return result of the call
     */
     ...
     * acquired elements before closing the session. Do not attempt to access
     * FIFO elements after the elements are released or the session is closed.
     *
     * @param session handle to a currently open session
     * @param fifo target-to-host FIFO from which to read
    ...
    /**
     * Acquires elements for reading from a target-to-host FIFO of unsigned
     * 8-bit integers.
     *
     * Acquiring, reading, and releasing FIFO elements prevents the need to copy
     * the contents of elements from the host memory buffer to a separate
     ...
     *
     * Acquiring, writing, and releasing FIFO elements prevents the need to
     * write first into a separate user-allocated buffer and then copy the
     * contents of elements to the host memory buffer. The FPGA target cannot
     * read elements acquired by the host. Therefore, the host must release
     * elements after writing to them. The number of elements acquired may
     * differ from the number of elements requested if, for example, the number
     * of elements requested reaches the end of the host memory buffer. Always
     * release all acquired elements before closing the session. Do not attempt
     * to access FIFO elements after the elements are released or the session is
     * closed.
     *
     * @param session handle to a currently open session
     * @param fifo host-to-target FIFO to which to write
     ...
     * host must release elements after acquiring them. Always release all
     * acquired elements before closing the session. Do not attempt to access
     * FIFO elements after the elements are released or the session is closed.
     *
     * @param session handle to a currently open session
